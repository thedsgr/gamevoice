import { Guild, VoiceChannel, CategoryChannel, ChannelType, GuildMember } from 'discord.js';
...
async function movePlayersToChannel(players: GuildMember[], channel: VoiceChannel): Promise<void> {
    await Promise.all(
        players.map(player =>
            player.voice.setChannel(channel).catch(err =>
                console.error(`Erro ao mover o jogador ${player.displayName} para o canal ${channel.name}:`, err)
            )
        )
    );
}
    ...
    const teamMembers = teamData.teamPlayers
        .map(player => {
            const member = waitingPlayers.find(m => 
                m.displayName.includes(player.riotName) || 
