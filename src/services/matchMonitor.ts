 ...
 */
export async function validatePlayersInSameMatch(players: { summonerId: string; discordId: string }[]): Promise<{
  matchId: number;
 ...
 */
export async function addPlayerToWaitingList(discordId: string, riotId: string): Promise<void> {
  try {
    const waitingList: { discordId: string; riotId: string; joinedAt: number }[] = (db.data?.waitingList as unknown as { discordId: string; riotId: string; joinedAt: number }[]) || [];
    waitingList.push({ discordId, riotId, joinedAt: Date.now() });
    db.data!.waitingList = waitingList as unknown as string[];
    await db.write();
...
function isValidSummonerName(name: string): boolean {
...
async function fetchTeamData(players: GuildMember[]): Promise<TeamData | null> {
  ...
  const teamDataArray = await getTeamFromRiotAPI(summonerNames) as TeamData[]; // Certifique-se de que getTeamFromRiotAPI retorna TeamData[]
  ...
  const mappedPlayers = (teamData.teamPlayers as unknown as TeamPlayer[]).map((player) => {
...
const players: GuildMember[] = []; // Replace with actual GuildMember array
const teamData = await fetchTeamData(players);
console.log(teamData);

const guild = await client.guilds.fetch(process.env.GUILD_ID ?? '');
const waitingRoom = guild.channels.cache.get(process.env.WAITING_ROOM_ID ?? '') as VoiceChannel;
if (teamData) {
...
export function startTracking(
    ...
    checkMatchStatus: (matchId: string) => Promise<boolean>,
    endMatch: (matchId: string) => Promise<void>,
    forceEndMatch: (matchId: string) => Promise<void>,
    intervalMs: number = 30000 // Intervalo padrão de 30 segundos
): void {
    ...
    const timeout = setTimeout(async () => {
        try {
            ...
            Logger.error(`Erro ao forçar encerramento da partida ${matchData.matchId}`, error as Error);
        }
    }, 2 * 60 * 60 * 1000);
