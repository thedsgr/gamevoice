...
function getVoiceChannels(guild) {
    return [...guild.channels.cache.filter((c) => c.type === ChannelType.GuildVoice).values()];
}
/* ==========================
...
export async function validatePlayersInSameMatch(players) {
    try {
        ...
        Logger.error('Falha na validação da partida:', error);
        return null;
    }
}
/**
 ...
 */
export async function startMatch(guild, lobbyChannel, players) {
    if (players.length < 2) {
    ...
    try {
        ...
        const stopMonitoring = monitorLobby(guild.client, lobbyChannel.id, async (updatedPlayers) => {
            ...
            if (updatedPlayers.length >= 2) {
                ...
                stopMonitoring();
                const riotAccounts = await Promise.all(updatedPlayers.map(async (p) => {
                ...
                const validPlayers = updatedPlayers.filter((p) => p.voice.channel).map((p) => p);
                if (validPlayers.length < 2) {
                    Logger.info('Número insuficiente de jogadores com canais de voz válidos');
                    return;
                }
                const matchInfo = await validatePlayersInSameMatch(riotAccounts.filter((account) => account !== null));
                if (!matchInfo) {
...
export async function endMatch(matchId) {
    ...
    try {
        ...
        Logger.error(`Erro ao finalizar partida ${matchId}:`, error);
        return false;
    }
}
/* ==========================
...
async function createTeamChannels(guild) {
    ...
    const team1 = await guild.channels.create({
        ...
        parent: category.id,
        userLimit: 5
    }); // Garante que o TypeScript trate como VoiceChannel
 ...
 */
async function movePlayersToTeams(players, team1, team2) {
    if (players.length <= 5) {
 ...
 */
async function movePlayersToLobby(matchData) {
    ...
    for (const channel of teamChannels) {
        ...
        for (const member of members) {
            try {
                ...
                Logger.error(`Erro ao mover jogador ${member.user.tag} para o lobby:`, error);
            }
        }
    }
}
 ...
 */
async function processVoiceChannels(guild) {
 ...
 */
export async function monitorPlayerStatus(guild, lobbyChannel) {
 ...
 */
export async function moveToTeamVoiceChannel(playerId, gameId) {
    try {
        ...
        const teamChannel = guild.channels.cache.find((channel) => channel.name === `Time-${gameId}` && channel.isVoiceBased());
        if (!teamChannel) {
        ...
        Logger.error(`Erro ao mover jogador para o canal do time:`, error);
        throw error;
    }
}
